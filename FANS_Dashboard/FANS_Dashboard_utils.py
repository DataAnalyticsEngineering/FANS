        ...
        for key in group.keys():
            ...
            if isinstance(item, h5py.Group):
                if any(subkey.startswith("load") for subkey in item.keys()):
                    microstructures[new_path] = recursively_find_structure(
                        item, new_path
                    )
                else:
...
def summarize_hierarchy(hierarchy):
    ...
    for microstructure, loads in hierarchy.items():
        load_cases = {
            key: value for key, value in loads.items() if key.startswith("load")
        }
        load_case_count = len(load_cases)
        print(
            f"  Microstructure '{microstructure}' has {load_case_count} load case(s):"
        )

        for load_case, time_steps in load_cases.items():
            time_step_count = len(time_steps)
            print(
                f"    Load case '{load_case}' has {time_step_count} time step(s) and the following quantities:"
            )
...
def extract_and_organize_data(
    ...
    hierarchy,
    quantities_to_load,
    microstructures_to_load=None,
    load_cases_to_load=None,
    time_steps_to_load=None,
):
    """
    ...
    with h5py.File(file_path, "r") as h5file:
        for microstructure, loads in hierarchy.items():
            if (
                ...
                and microstructure not in microstructures_to_load
            ):
            ...
            for load_case, time_steps in loads.items():
                ...
                sorted_time_steps = sorted(
                    time_steps.items(), key=lambda x: int(x[0].replace("time_step", ""))
                )
                ...
                for time_step, _ in sorted_time_steps:
                    ...
                    if (
                        time_steps_to_load
                        and time_step_number not in time_steps_to_load
                    ):
                    ...
                    for quantity in quantities_to_load:
                        if quantity in time_steps[time_step]:
                            data_array = np.array(
                                h5file[
                                    f"{microstructure}/{load_case}/{time_step}/{quantity}"
                                ]
                            )
                ...
                for quantity, data_list in extracted_data.items():
                    if data_list:
                        organized_data[microstructure][load_case][quantity] = np.stack(
                            data_list, axis=0
                        )

                organized_data[microstructure][load_case]["time_steps"] = np.array(
                    time_step_values
                )
            ...
            for load_case, data in loads.items():
                ...
                for measure_name, data_array in data.items():
                    for time_step_idx, time_step in enumerate(time_steps):
                        time_step_group = (
                            f"{microstructure}/{load_case}/time_step{time_step}"
                        )
                        ...
                        if time_step_group not in h5file:
                            raise ValueError(
                                f"Group '{time_step_group}' does not exist in the HDF5 file."
                            )
                        ...
                        if dataset_path in h5file:
                            if overwrite:
                                ...
                                h5file.create_dataset(
                                    dataset_path, data=data_array[time_step_idx]
                                )
                            else:
                                print(
                                    f"Dataset exists and overwrite=False: {dataset_path}"
                                )
                        else:
                            h5file.create_dataset(
                                dataset_path, data=data_array[time_step_idx]
...
def postprocess_and_write_to_h5(
    file_path,
    hierarchy,
    quantities_to_process,
    measures,
    microstructures_to_load=None,
    load_cases_to_load=None,
):
    ...
    extracted_data = extract_and_organize_data(
        file_path,
        hierarchy,
        quantities_to_process,
        microstructures_to_load,
