...
def write_xdmf(
    ...
    def dataset_to_xdmf(dset, grid, time=None):
        ...
        if dimensions == 3:
            ...
            print_verbose(
                f"Omitting dataset {dset.name} due to unexpected number of dimensions: {dimensions}"
            )
            return
        ...
        if time_series:
            ...
            if time_index:
                ...
                attr_name = "/".join(
                    attr_name_parts[:time_index] + attr_name_parts[time_index + 1 :]
                )
            else:
                attr_name = dset.name
        else:
        ...
        print_verbose(
            f'Converted dataset {dset.name} to XDMF and added to grid {grid.get("Name")}.'
        )
        ...
        for dset in group.values():
            if isinstance(dset, h5py.Dataset):
                if len(dset.shape) in [3, 4]:
                    ...
                    print_verbose(
                        f"Omitting dataset {dset.name} due to unexpected number of dimensions: {len(dset.shape)}"
                    )
    ...
    with h5py.File(h5_filepath, "r") as h5_file:
        ...
        for (grid_size, time), dsets in list(grid_dict.items()):
            Nx, Ny, Nz = grid_size
            DX, DY, DZ = (
                microstructure_length[0] / Nx,
                microstructure_length[1] / Ny,
                microstructure_length[2] / Nz,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "h5_filepath", type=str, nargs="+", help="Path(s) to the input HDF5 file(s)."
    )
    parser.add_argument(
        "-x",
        "--xdmf_filepath",
