...
def mandel_to_matrix(mandel_tensor):
    ...
    full_tensor[:, 0, 1] = full_tensor[:, 1, 0] = mandel_tensor[:, 3] / np.sqrt(2)  # s12 or e12
    full_tensor[:, 0, 2] = full_tensor[:, 2, 0] = mandel_tensor[:, 4] / np.sqrt(2)  # s13 or e13
    full_tensor[:, 1, 2] = full_tensor[:, 2, 1] = mandel_tensor[:, 5] / np.sqrt(2)  # s23 or e23
    return full_tensor
...
def matrix_to_mandel(full_tensor, tolerance=1e-8):
    ...
    if not np.allclose(full_tensor, full_tensor.transpose(0, 2, 1), atol=tolerance):
        raise ValueError("One or more tensors are not symmetric within the specified tolerance.")
