...
using Array1D = nb::ndarray<const double, nb::ndim<1>>;

std::vector<double> conv_to_vector(const nb::object &obj, const int size)
{
    const auto          arr = nb::cast<Array1D>(obj);
    std::vector<double> res;
    res.resize(size);
...
static nb::ndarray<nb::numpy, double, nb::shape<3>> make_np_array(double d0, double d1, double d2)
{
...
nb::dict MicroSimulation::solve(const nb::dict &macro_data, double dt)
{
    ...
    if (not is_small_strain) {
        std::vector<double> strain3 = conv_to_vector(macro_data["Strains7to9"], 3);
    ...
    if (macro_data.contains("ComputeTangent"))
        fresh = nb::cast<double>(macro_data["ComputeTangent"]) != 0.0;
...
NB_MODULE(PyFANS, m)
{
    ...
    nb::class_<MicroSimulation>(m, "MicroSimulation")
        .def(nb::init<int, bool, const std::string &, const std::string &>(),
             nb::arg("sim_id"),
