...
app = marimo.App(width="medium", layout_file="layouts/fans_input.grid.json")
...
def __(change_input, mo):
    ...
    def create_array_input(name, length, values, start=-1e5, stop=1e5, on_change=None, **kwargs):
        t = []
        vals = {}
...
def __(create_array_input, mo, ui_number_of_phases):
    ...
    def create_nested_array_input(name, subnames, values, on_change, start=-1e5, stop=1e5, **kwargs):
        t = []
        for i, subname in enumerate(subnames):
            # print(f"Creating array for {name}, with {subname}, current value:{values[subname]}")
            t1 = create_array_input(subname, ui_number_of_phases.value, values[subname], on_change=on_change(subname))
            t.append(t1)
...
def __():
    ...
    input_standard = {"comment": "This field is ignored and can be used to describe the format. 'problem_type' can be 'thermal' or 'mechanical'. 'method' can be 'fp' or 'cg'. 'matmodel' can be 'ThermalLinear', 'MechLinear' or 'HyperElastic'. g0 is the macroscale loading vectors - each row is a new loading condition",
...
def __(mo):
    ...
    ui_number_of_phases = mo.ui.number(start=2, stop=6, value=2, label="Number of Phases")
...
def __(change_input, create_array_input, get_input_value, mo):
    # General ui elements
    ui_ms_filename = mo.ui.text(value=get_input_value("ms_filename"), label="ms_filename", on_change=change_input("ms_filename"), max_length=100)
    ui_problem_dropdown = mo.ui.dropdown(
        options=["mechanical", "thermal"], value=get_input_value("problem_type"), label="Problem type:", on_change=change_input("problem_type")
    )
    ...
    match ui_problem_dropdown.value:
        case "mechanical":
            ui_matmodel_dropdown = mo.ui.dropdown(
                    options=["MechLinear", "HyperElastic"], value=get_input_value("matmodel"), label="Material Model:", on_change=change_input("matmodel")
                )
        case "thermal":
...
def __(
    ...
    match get_fans_input()["matmodel"]:
        case "MechLinear":
            subnames = ["bulk_modulus", "shear_modulus", "critical_stress", "hardening_parameter"]
            ui_matmodel_parameters = create_nested_array_input("material_properties", subnames, values=get_input_value("material_properties"), on_change=change_material )
        case "HyperElastic":
            pass
