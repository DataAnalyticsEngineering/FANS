...
def test_displacement_averaging(test_files):
    ...
    -------
    None
        The test passes if the average of displacement fluctuations is zero.
    """
        ...
        for load_case in load_cases_to_load:
            if load_case in hierarchy[microstructure]:
                if 'displacement' not in data[microstructure][load_case]:
                    print(f"Skipping {microstructure}/{load_case}: Missing displacement field")
                    continue
                ...
                assert np.allclose(computed_average, zero_array, rtol=1e-5, atol=1e-8), \
                ...
                print(f"Verified: {microstructure}, load case {load_case} - displacement fluctuation average is zero")
