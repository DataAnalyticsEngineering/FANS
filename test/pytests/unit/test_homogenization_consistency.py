...
def test_homogenization_consistency(test_files):
    ...
    if missing_fields:
        pytest.skip(f"Skipping test: Missing required fields {', '.join(missing_fields)} in {input_json_file}")
    ...
    for microstructure in microstructures_to_load:
        for load_case in load_cases_to_load:
            if load_case in hierarchy[microstructure]:
                ...
                required_data = {field: field in data[microstructure][load_case] 
                               for field in quantities_to_load}
                
                if not all(required_data.values()):
                    missing = [field for field, exists in required_data.items() if not exists]
                    print(f"Skipping {microstructure}/{load_case}: Missing fields: {', '.join(missing)}")
                    continue
                ...
                for t in range(len(strain_avg)):
                    ...
                    assert np.allclose(computed_stress, stress_avg[t], rtol=1e-4, atol=1e-1), \
                ...
                print(f"Verified: {microstructure}, load case {load_case} - " 
