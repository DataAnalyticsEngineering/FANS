...
def test_strain_stress_averaging(test_files):
        ...
        -------
        None
            The test passes if the manually computed averages match the stored averages.
        """
        ...
        if not fields_to_check:
            pytest.skip(f"Skipping test: No compatible strain/stress and average pairs found in {input_json_file}")
            return
            ...
            for microstructure in microstructures_to_load:
                for load_case in load_cases_to_load:
                    if load_case in hierarchy[microstructure]:
                        if field not in data[microstructure][load_case] or avg_field not in data[microstructure][load_case]:
                            print(f"Skipping {microstructure}/{load_case}: Missing {field} or {avg_field}")
                            continue
                        ...
                        assert np.allclose(computed_average, avg_field_data, rtol=1e-5, atol=1e-8), \
                        ...
                        print(f"Verified: {microstructure}, load case {load_case} - {field} average matches {avg_field}")
