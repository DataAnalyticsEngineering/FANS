  ...
  public:
    ...
    MaterialManager(const Reader &reader)
    {
        ...
        for (size_t i = 0; i < mats.size(); ++i) {
            ...
            Log::io->info() << Log::format("# Material Model %zu: %s\n", i + 1, mg["matmodel"].get<string>().c_str());

            // Print phases
            auto phases = mg["phases"].get<vector<int>>();
            Log::io->info() << "#   Phases: [";
            for (size_t j = 0; j < phases.size(); ++j) {
                Log::io->info(true) << Log::format("%d", phases[j]);
                if (j < phases.size() - 1)
                    Log::io->info(true) << ", ";
            }
            Log::io->info(true) << "]\n";
            ...
            for (auto it = props.begin(); it != props.end(); ++it) {
                Log::io->info() << Log::format("#     %s: ", it.key().c_str());
                if (it.value().is_array()) {
                    Log::io->info(true) << "[";
                    for (size_t k = 0; k < it.value().size(); ++k) {
                        if (it.value()[k].is_number()) {
                            Log::io->info(true) << Log::format("%.5g", it.value()[k].get<double>());
                        } else if (it.value()[k].is_string()) {
                            Log::io->info(true) << Log::format("\"%s\"", it.value()[k].get<string>().c_str());
                        }
                    ...
                    Log::io->info(true) << Log::format("%.5g", it.value().get<double>());
                } else if (it.value().is_string()) {
                    Log::io->info(true) << Log::format("\"%s\"", it.value().get<string>().c_str());
                }
                Log::io->info(true) << "\n";
            }
            Log::io->info() << "#\n";
    ...
    void compute_reference_stiffness(const Reader &reader)
    {
        if (reader.inputJson.contains("reference_material")) {
            ...
            Log::io->info() << "# Using user-defined reference material for fundamental solution.\n";
        } else {
