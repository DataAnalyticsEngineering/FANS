...
void SolverCG<howmany, n_str>::LineSearchSecant()
{
    ...
    Log::solver->info() << "line search iter " << _iter
                        << ", alpha " << std::fixed << std::setprecision(6) << alpha_curr << std::defaultfloat
                        << " - error " << std::fixed << std::setprecision(6) << (fabs(r1pd0) > 0.0 ? fabs(r1pd) / fabs(r1pd0) : 0.0) << std::defaultfloat << " - ";
}
